...
Select Location, population,MAX(total_deaths) as HighestDeathCount,MAX(Total_deaths*1.0 / population*1.0) *100 as PercentPopulationDeath
From [master].[dbo].[covid_deaths]
Where continent is not null 
Group by Location,population
...
From [master].[dbo].[covid_deaths] dea
Join [master].[dbo].[covid_vaccinations] vac
...
Select *, (RollingPeopleVaccinated*1.0/Population*1.0)*100 as TotalVaccinationPerPopulation
From PopvsVac
...
-- Query of tableau modules

-- 1.  global numbers( total cases and total deaths and percentage )
Select SUM(new_cases) as total_cases, SUM(new_deaths)  as total_deaths, (SUM(new_deaths) *1.0/SUM(New_Cases)*1.0)*100 as DeathPercentage
From [master].[dbo].[covid_deaths]
where continent is not null 
order by 1,2

-- 2.  highest number of death per continent
...
--excluding wronge values
and location not in ('World', 'European Union', 'International','High income','Upper middle income','Lower middle income','Low income') 
Group by location
order by TotalDeathCount desc


-- 3. total infections by country
Select Location, Population, MAX(total_cases) as HighestInfectionCount,  Max((total_cases*1.0/population*1.0))*100 as PercentPopulationInfected
From [master].[dbo].[covid_deaths]
...
Select Location, Population,date, MAX(total_cases) as HighestInfectionCount,  Max((total_cases*1.0/population*1.0))*100 as PercentPopulationInfected
From [master].[dbo].[covid_deaths]
